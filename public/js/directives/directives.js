angular.module('mainApp').directive("keyValue", function() {
    return {
        ...
        template: ['<div class="form-group keyValue">'+'<label class="keyValue">*Key: </label>'+
            '<input ng-class="{red: !item.key && editWidgetForm.key.$dirty}" id=key{{$index+1}} name=key ng-model="item.key" class="col-xs-5 col-md-5 keyValueStyle" type="text" class="form-control" placeholder="Enter key{{$index+1}}" required>'+
            ...
            '<p class="HelpMessage" ng-show="!item.key && editWidgetForm.key.$dirty ">Please enter value for key{{$index+1}}</p>'+
            '<p class="HelpMessage" ng-show="!checkUniqueKey(item.key || editWidgetForm.key.$dirty)">This key name is already taken in this widget, please choose another name</p>'
        ].join()
    };
});
